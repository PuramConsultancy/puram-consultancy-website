...
export const buttonVariants = cva(
  "group relative cursor-pointer text-shadow-white/30 text-shadow-xs rounded-[var(--rounded)] text-sm font-medium transition-all duration-300 [--rounded:var(--radius-xl)] disabled:text-[var(--color-btn-text-disabled)] disabled:bg-[var(--color-btn-disabled)] disabled:cursor-not-allowed",
  {
    variants: {
      variant: {
        primary:
          "bg-[var(--color-secondary)] text-white hover:bg-[var(--color-secondary-500)]",
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "group relative overflow-hidden p-3 focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:outline-none",
        )}
        disabled={isLoading}
        {...props}
      >
        ...
        <div className="relative z-30 flex items-center justify-center gap-2">
          {!isLoading ? children : <Spinner className="size-4" />}
        </div>
      </button>
    );
  },
