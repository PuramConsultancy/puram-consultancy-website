"use client";

import { useState, useMemo } from "react";
import { useRouter } from "next/navigation";
import { motion, AnimatePresence } from "motion/react";
import {
  IoSearch,
  IoClose,
  IoPersonCircle,
  IoCalendar,
  IoCheckmarkCircle,
  IoDocumentText,
  IoChevronForward,
  IoChevronDown,
  IoArrowForward,
  IoDownload,
} from "react-icons/io5";
import {
  useGetAllSubmissions,
  FormWithSubmissions,
} from "@/app/api-client/forms/useGetAllSubmissions";
import {
  Submission,
  FormQuestion,
} from "@/app/api-client/forms/useGetSubmissions";
import {
  exportFormSubmissions,
  exportAllSubmissions,
} from "@/app/api-client/forms/utils/exportSubmissions";

// ── Helpers ───────────────────────────────────────────────────────────────────

const formatDate = (iso: string) =>
  new Date(iso).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
    hour: "2-digit",
    minute: "2-digit",
  });

const getInitials = (s: Submission) => {
  if (s.user?.firstName)
    return `${s.user.firstName[0]}${s.user.lastName?.[0] ?? ""}`.toUpperCase();
  return "G";
};

const getDisplayName = (s: Submission) => {
  if (s.user?.firstName)
    return `${s.user.firstName} ${s.user.lastName ?? ""}`.trim();
  return "Guest";
};

const AVATAR_COLORS = [
  "bg-violet-500",
  "bg-sky-500",
  "bg-emerald-500",
  "bg-rose-500",
  "bg-amber-500",
  "bg-indigo-500",
  "bg-pink-500",
  "bg-teal-500",
];

const avatarColor = (id: string) =>
  AVATAR_COLORS[
    id.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % AVATAR_COLORS.length
  ];

const flatQuestions = (f: FormWithSubmissions): FormQuestion[] =>
  f.sections.flatMap((s) => s.questions);

const renderAnswer = (val: unknown): string => {
  if (val === undefined || val === null || val === "") return "—";
  if (Array.isArray(val)) return val.join(", ") || "—";
  return String(val);
};

// ── Detail Drawer ─────────────────────────────────────────────────────────────

const SubmissionDrawer = ({
  submission,
  questions,
  onClose,
}: {
  submission: Submission | null;
  questions: FormQuestion[];
  onClose: () => void;
}) => (
  <AnimatePresence>
    {submission && (
      <>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
          className="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm"
        />
        <motion.aside
          initial={{ x: "100%" }}
          animate={{ x: 0 }}
          exit={{ x: "100%" }}
          transition={{ type: "spring", damping: 28, stiffness: 260 }}
          className="fixed top-0 right-0 z-50 flex h-full w-full max-w-lg flex-col bg-white shadow-2xl"
        >
          <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
            <div className="flex items-center gap-3">
              <div
                className={`flex size-10 items-center justify-center rounded-xl text-sm font-bold text-white ${avatarColor(submission.id)}`}
              >
                {getInitials(submission)}
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-800">
                  {getDisplayName(submission)}
                </p>
                {submission.user?.email && (
                  <p className="text-xs text-gray-400">
                    {submission.user.email}
                  </p>
                )}
              </div>
            </div>
            <button
              onClick={onClose}
              className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
            >
              <IoClose className="size-5" />
            </button>
          </div>

          <div className="flex items-center gap-4 border-b border-gray-100 px-6 py-3">
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <IoCalendar className="size-3.5" />
              {formatDate(submission.createdAt)}
            </div>
            <div className="flex items-center gap-1.5 text-xs font-medium text-emerald-600">
              <IoCheckmarkCircle className="size-3.5" />
              Submitted
            </div>
          </div>

          <div className="flex-1 overflow-y-auto px-6 py-5">
            <p className="mb-4 text-xs font-bold tracking-widest text-gray-400 uppercase">
              Responses
            </p>
            <div className="flex flex-col gap-4">
              {questions.map((q, i) => (
                <motion.div
                  key={q.id}
                  initial={{ opacity: 0, y: 6 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.04 }}
                  className="rounded-xl border border-gray-100 bg-gray-50 p-4"
                >
                  <p className="text-xs font-semibold text-gray-400">
                    {q.label}
                  </p>
                  <p className="mt-1.5 text-sm font-medium break-words text-gray-800">
                    {renderAnswer(
                      (submission.data as Record<string, unknown>)[q.id],
                    )}
                  </p>
                </motion.div>
              ))}
            </div>
          </div>

          <div className="border-t border-gray-100 px-6 py-4">
            <p className="text-[10px] text-gray-300">
              Submission ID: {submission.id}
            </p>
          </div>
        </motion.aside>
      </>
    )}
  </AnimatePresence>
);

// ── Form Group ────────────────────────────────────────────────────────────────

const FormGroup = ({
  form,
  globalSearch,
  onSelect,
  onViewAll,
  defaultOpen,
}: {
  form: FormWithSubmissions;
  globalSearch: string;
  onSelect: (s: Submission, qs: FormQuestion[]) => void;
  onViewAll: () => void;
  defaultOpen: boolean;
}) => {
  const [open, setOpen] = useState(defaultOpen);
  const questions = useMemo(() => flatQuestions(form), [form]);

  const filtered = useMemo(() => {
    if (!globalSearch) return form.submissions;
    const q = globalSearch.toLowerCase();
    return form.submissions.filter((s) => {
      const name = getDisplayName(s).toLowerCase();
      const email = s.user?.email?.toLowerCase() ?? "";
      return name.includes(q) || email.includes(q);
    });
  }, [form.submissions, globalSearch]);

  if (globalSearch && filtered.length === 0) return null;

  return (
    <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
      {/* Form header row */}
      <div
        onClick={() => setOpen((v) => !v)}
        className="flex w-full cursor-pointer items-center justify-between gap-3 px-5 py-4 transition-colors hover:bg-gray-50"
      >
        <div className="flex items-center gap-3">
          <div className="flex size-9 items-center justify-center rounded-xl bg-violet-100">
            <IoDocumentText className="size-4 text-violet-600" />
          </div>
          <div className="text-left">
            <p className="text-sm font-semibold text-gray-800">{form.title}</p>
            {form.description && (
              <p className="text-xs text-gray-400">{form.description}</p>
            )}
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-600">
            {form.submissions.length} submission
            {form.submissions.length !== 1 ? "s" : ""}
          </span>

          {/* Export */}
          <div
            onClick={(e) => {
              e.stopPropagation();
              exportFormSubmissions(
                form.title,
                form.submissions,
                flatQuestions(form),
              );
            }}
            className="flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-gray-400 transition-colors hover:bg-emerald-50 hover:text-emerald-600"
          >
            <IoDownload className="size-3.5" />
            Export
          </div>

          {/* View all */}
          <div
            onClick={(e) => {
              e.stopPropagation();
              onViewAll();
            }}
            className="flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-gray-400 transition-colors hover:bg-violet-50 hover:text-violet-600"
          >
            View all
            <IoArrowForward className="size-3.5" />
          </div>

          <motion.span
            animate={{ rotate: open ? 180 : 0 }}
            transition={{ duration: 0.2 }}
          >
            <IoChevronDown className="size-4 text-gray-400" />
          </motion.span>
        </div>
      </div>

      {/* Submissions table */}
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            initial={{ height: 0, opacity: 0 }}
            animate={{ height: "auto", opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            transition={{ duration: 0.22, ease: "easeInOut" }}
            className="overflow-hidden"
          >
            <div className="border-t border-gray-100">
              {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  <p className="text-sm text-gray-400">No submissions yet.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/60">
                        <th className="px-5 py-3 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">
                          Submitter
                        </th>
                        {questions.slice(0, 3).map((q) => (
                          <th
                            key={q.id}
                            className="px-5 py-3 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase"
                          >
                            {q.label.length > 18
                              ? q.label.slice(0, 18) + "…"
                              : q.label}
                          </th>
                        ))}
                        <th className="px-5 py-3 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">
                          Date
                        </th>
                        <th className="px-5 py-3 text-left text-xs font-semibold tracking-wider text-gray-400 uppercase">
                          Status
                        </th>
                        <th className="w-10 px-5 py-3" />
                      </tr>
                    </thead>
                    <tbody>
                      {filtered.map((submission, i) => (
                        <motion.tr
                          key={submission.id}
                          initial={{ opacity: 0, y: 4 }}
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: i * 0.03 }}
                          onClick={() => onSelect(submission, questions)}
                          className="group cursor-pointer border-b border-gray-50 transition-colors last:border-0 hover:bg-violet-50/40"
                        >
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-3">
                              <div
                                className={`flex size-8 shrink-0 items-center justify-center rounded-xl text-xs font-bold text-white ${avatarColor(submission.id)}`}
                              >
                                {getInitials(submission)}
                              </div>
                              <div>
                                <p className="font-medium text-gray-800">
                                  {getDisplayName(submission)}
                                </p>
                                {submission.user?.email && (
                                  <p className="text-xs text-gray-400">
                                    {submission.user.email}
                                  </p>
                                )}
                              </div>
                            </div>
                          </td>

                          {questions.slice(0, 3).map((q) => (
                            <td
                              key={q.id}
                              className="max-w-[160px] truncate px-5 py-3.5 text-gray-600"
                            >
                              {renderAnswer(
                                (submission.data as Record<string, unknown>)[
                                  q.id
                                ],
                              )}
                            </td>
                          ))}

                          <td className="px-5 py-3.5 text-xs whitespace-nowrap text-gray-400">
                            {formatDate(submission.createdAt)}
                          </td>

                          <td className="px-5 py-3.5">
                            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-600">
                              <span className="size-1.5 rounded-full bg-emerald-500" />
                              Submitted
                            </span>
                          </td>

                          <td className="px-5 py-3.5">
                            <IoChevronForward className="size-4 text-gray-300 transition-colors group-hover:text-violet-500" />
                          </td>
                        </motion.tr>
                      ))}
                    </tbody>
                  </table>
                </div>
              )}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
};

// ── Main Page ─────────────────────────────────────────────────────────────────

const AllSubmissionsPage = () => {
  const router = useRouter();
  const { data, isLoading, isError } = useGetAllSubmissions();
  const forms = data?.data ?? [];

  const [search, setSearch] = useState("");
  const [selected, setSelected] = useState<{
    submission: Submission;
    questions: FormQuestion[];
  } | null>(null);

  const totalSubmissions = forms.reduce((a, f) => a + f.submissions.length, 0);
  const todayCount = forms.reduce(
    (a, f) =>
      a +
      f.submissions.filter(
        (s) =>
          new Date(s.createdAt).toDateString() === new Date().toDateString(),
      ).length,
    0,
  );
  const formsWithSubmissions = forms.filter(
    (f) => f.submissions.length > 0,
  ).length;

  // ── Loading ──
  if (isLoading) {
    return (
      <section className="flex h-full flex-1 flex-col overflow-hidden">
        <div className="h-full overflow-y-auto bg-gray-50 p-5">
          <div className="flex flex-col gap-4">
            <div className="h-8 w-48 animate-pulse rounded-lg bg-gray-200" />
            <div className="grid grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-24 animate-pulse rounded-2xl bg-gray-200"
                />
              ))}
            </div>
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-40 animate-pulse rounded-2xl bg-gray-200"
              />
            ))}
          </div>
        </div>
      </section>
    );
  }

  // ── Error ──
  if (isError) {
    return (
      <section className="flex h-full flex-1 items-center justify-center">
        <p className="text-sm text-gray-500">Failed to load submissions.</p>
      </section>
    );
  }

  return (
    <>
      <section className="flex h-full flex-1 flex-col overflow-hidden">
        <div className="h-full overflow-y-auto bg-gray-50 p-5">
          <div className="flex flex-col gap-5">
            {/* ── Page title + search + export all ── */}
            <div className="flex items-center justify-between gap-4">
              <div>
                <h1 className="text-lg font-semibold text-gray-800">
                  All Submissions
                </h1>
                <p className="mt-0.5 text-xs text-gray-400">
                  Submissions across {forms.length} form
                  {forms.length !== 1 ? "s" : ""}
                </p>
              </div>

              <div className="flex items-center gap-2">
                <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 ring-1 ring-gray-200">
                  <IoSearch className="size-4 text-gray-400" />
                  <input
                    placeholder="Search by name or email…"
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-52 bg-transparent text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none"
                  />
                  {search && (
                    <button onClick={() => setSearch("")}>
                      <IoClose className="size-3.5 text-gray-400 hover:text-gray-600" />
                    </button>
                  )}
                </div>

                <button
                  onClick={() => exportAllSubmissions(forms)}
                  disabled={totalSubmissions === 0}
                  className="flex items-center gap-1.5 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-600 transition-colors hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <IoDownload className="size-3.5" />
                  Export All
                </button>
              </div>
            </div>

            {/* ── Stat cards ── */}
            <div className="grid grid-cols-3 gap-4">
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-500 to-violet-700 p-5">
                <div className="pointer-events-none absolute -top-4 -right-4 size-20 rounded-full bg-white/10" />
                <IoDocumentText className="size-5 text-white/80" />
                <p className="mt-3 text-2xl font-bold text-white">
                  {totalSubmissions}
                </p>
                <p className="mt-0.5 text-xs font-medium text-white/70">
                  Total Submissions
                </p>
              </div>
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-sky-500 to-sky-700 p-5">
                <div className="pointer-events-none absolute -top-4 -right-4 size-20 rounded-full bg-white/10" />
                <IoCalendar className="size-5 text-white/80" />
                <p className="mt-3 text-2xl font-bold text-white">
                  {todayCount}
                </p>
                <p className="mt-0.5 text-xs font-medium text-white/70">
                  Today
                </p>
              </div>
              <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-700 p-5">
                <div className="pointer-events-none absolute -top-4 -right-4 size-20 rounded-full bg-white/10" />
                <IoPersonCircle className="size-5 text-white/80" />
                <p className="mt-3 text-2xl font-bold text-white">
                  {formsWithSubmissions}
                </p>
                <p className="mt-0.5 text-xs font-medium text-white/70">
                  Active Forms
                </p>
              </div>
            </div>

            {/* ── Empty state ── */}
            {forms.length === 0 && (
              <div className="flex flex-col items-center justify-center rounded-2xl bg-white py-20 shadow-sm ring-1 ring-gray-100">
                <div className="flex size-14 items-center justify-center rounded-2xl bg-gray-100">
                  <IoDocumentText className="size-7 text-gray-300" />
                </div>
                <p className="mt-4 text-sm font-medium text-gray-500">
                  No forms yet.
                </p>
                <button
                  onClick={() => router.push("/admin-dashboard/forms")}
                  className="mt-3 text-sm font-medium text-violet-600 hover:underline"
                >
                  Create your first form →
                </button>
              </div>
            )}

            {/* ── Form groups ── */}
            {forms.map((form, i) => (
              <FormGroup
                key={form.id}
                form={form}
                globalSearch={search}
                defaultOpen={i === 0}
                onSelect={(submission, questions) =>
                  setSelected({ submission, questions })
                }
                onViewAll={() =>
                  router.push(`/admin-dashboard/forms/${form.id}/submissions`)
                }
              />
            ))}
          </div>
        </div>
      </section>

      <SubmissionDrawer
        submission={selected?.submission ?? null}
        questions={selected?.questions ?? []}
        onClose={() => setSelected(null)}
      />
    </>
  );
};

export default AllSubmissionsPage;